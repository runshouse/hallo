...
from hallo.models import (UNet2DConditionModel, UNet3DConditionModel,
                          FaceLocator, ImageProjModel, AudioProjModel)
from hallo.datasets import ImageProcessor, AudioProcessor
from hallo.animate import FaceAnimatePipeline
...
class Net(nn.Module):
    def __init__(self, reference_unet, denoising_unet, face_locator, imageproj, audioproj):
        ...
        # Freeze the models to prevent gradients
        self.reference_unet.requires_grad_(False)
        self.denoising_unet.requires_grad_(False)
        self.face_locator.requires_grad_(False)
        self.imageproj.requires_grad_(False)
        self.audioproj.requires_grad_(False)

    def forward(self, image, audio_emb, other_params):
        # Define how your forward pass operates using the components
        pass  # Placeholder for the actual forward pass
...
def inference_process(args: argparse.Namespace):
    ...
    reference_unet = UNet2DConditionModel.from_pretrained(config.base_model_path).to(device)
    denoising_unet = UNet3DConditionModel.from_pretrained(config.base_model_path).to(device)
    face_locator = FaceLocator().to(device)
    image_proj = ImageProjModel().to(device)
    audio_proj = AudioProjModel().to(device)

    # Network instance
    net = Net(reference_unet, denoising_unet, face_locator, image_proj, audio_proj)
    net.to(device=device, dtype=torch.float16)  # Using float16 to reduce memory usage

    # Load data and perform operations as needed
    # Example loading and processing, adjust according to your needs
    image_processor = ImageProcessor()
    audio_processor = AudioProcessor()
    source_image, _ = image_processor.load_image(args.image_path)
    audio_emb, _ = audio_processor.load_audio(args.audio_path)

    # Example forward pass
    net(source_image, audio_emb, None)

    # Cleanup to reduce memory footprint
    del net, reference_unet, denoising_unet, face_locator, image_proj, audio_proj
    torch.cuda.empty_cache()
    gc.collect()

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=str, default="configs/inference/default.yaml")
